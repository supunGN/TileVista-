...
export class ProductsService {
  ...
  private buildUnifiedItem(
    ...
    if (dbProduct && dbProduct.product_assets) {
      ...
      // 2. Map Material (prefer local db asset override, fallback to OSPOS attribute)
      dto.material = asset.material_type || osposItem.attributes?.['Material'] || osposItem.attributes?.['material'] || null;
      
      // 3. Map Color/Finish (prefer local db asset override, fallback to OSPOS attribute)
      dto.finish = asset.color_family || osposItem.attributes?.['Color'] || osposItem.attributes?.['color'] || null;
      ...
      // 2. Fallbacks for material/finish if no visual assets registered yet
      dto.material = osposItem.attributes?.['Material'] || osposItem.attributes?.['material'] || null;
      dto.finish = osposItem.attributes?.['Color'] || osposItem.attributes?.['color'] || null;
