...
function DynamicFurnitureModel({ item, selected }: { item: any, selected: boolean }) {
    ...
    <>
      ...
      <mesh geometry={floorGeom} rotation={[-Math.PI / 2, 0, 0]} position={[0, 0, 0]} receiveShadow>
        <meshStandardMaterial
          map={state.designType === 'room' ? woodTexture : floorTexture}
      ...
      {walls.map((wall, i) => {
        ...
        const getMat = (color: string, w: number, y: number) => {
          return state.designType === 'room'
            ? { color, roughness: 0.95, metalness: 0.05, side: THREE.DoubleSide, transparent: true }
            : { map: getTileTexture(color, Math.max(1, Math.round(w * 0.8)), Math.max(1, Math.round(y * 0.8))), roughness: 0.4, side: THREE.DoubleSide, transparent: true };
        };
        ...
        return (
          ...
          >
            ...
            {!['sink', 'bathtub', 'shower', 'toilet', 'towel_rail', 'washing_machine', 'light', 'plant'].includes(item.type) && (
              <DynamicFurnitureModel item={item} selected={isSelected} />
            )}
          </group>
        );
      })}
      ...
      {isPlacingItem && (
        <group position={isPlacingItem.position} rotation={[0, isPlacingItem.rotation, 0]} scale={[0.3048, 0.3048, 0.3048]}>
          ...
          {!['sink', 'bathtub', 'shower', 'toilet', 'towel_rail', 'washing_machine', 'light', 'plant'].includes(isPlacingItem.type) && (
            <DynamicFurnitureModel item={isPlacingItem} selected={true} />
          )}
        </group>
      )}
...
function RoomPreview3D({
  ...
  activePlacement: { type: 'door' | 'window'; style: string; name: string; width: number; height: number; sillHeight: number } | null;
  setActivePlacement: (ap: any) => void; placedItems?: any[]; setPlacedItems?: any; isPlacingItem?: any; setIsPlacingItem?: any; selectedItemId?: string | null; setSelectedItemId?: any; recordHistory?: any; setOrbitEnabled?: any;
}) {
        ...
        return (
          <group key={idx}>
            ...
            >
              ...
              <meshStandardMaterial
                color={isThisWallDragged ? '#4ade80' : (showOpeningsEditor && activePlacement ? '#22c55e' : '#ffffff')}
                transparent
                opacity={isThisWallDragged ? 0.7 : (showOpeningsEditor && activePlacement && hoveredWall?.idx === idx ? 0.25 : 0)}
                depthWrite={false}
              />
            </mesh>
...
function BathroomPlannerPageInner() {
    ...
    const fetchSavedDesign = async () => {
      try {
        ...
        if (dimensionsUnit === 'cm') {
          setWidthInput(Math.round(data.width * 100).toString());
          setLengthInput(Math.round(data.length * 100).toString());
          setHeightInput(Math.round(data.height * 100).toString());
        } else {
          setWidthInput(data.width.toString());
          setLengthInput(data.length.toString());
          setHeightInput(data.height.toString());
        }

        // Set state dimensions
        const wFt = cmToFeet(data.width * 100);
        const dFt = cmToFeet(data.length * 100);
        const hFt = cmToFeet(data.height * 100);
        ...
        setState({
          ...
          subRoomType: data.type === 'room' ? 'living_room' : undefined,
                    ...
                    {!wizardCategory ? (
                      <>
                        <div className="space-y-2">
                          ...
                          <div className="grid grid-cols-2 gap-2.5">
                            {getActiveCategories(selectedRoomType, subRoomType).map((cat) => (
                              <button
                                key={cat.id}
